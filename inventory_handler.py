...
class InventoryHandler(BaseHandler):
    ...
    def handle(self, message, **kwargs):
        ...
        if match:
            return self.handle_place(match, db)
        match = SEARCH_PATTERN.match(message.lower())
        if match:
            return self.handle_search(match, message, db)
    ...
    def handle_button(self, data, **kwargs):
        ...
        msg = self.format_thing("he {} {} no longer {} the {}.", thing)
        answer = {
            'answer': 'T{}'.format(msg),
    ...
    def handle_list(self, match, db):
        ...
        matches = fuzzy_match(
            location, things, lambda thing: thing['location'],
    ...
    def handle_search(self, match, message, db):
        groups = match.groups()
        plural = groups[0]
        name = groups[1]
    ...
    def handle_place(self, match, db):
        groups = match.groups()
        ...
        if old_thing:
            ...
            msg = self.format_thing("The {} {} now {} the {}.", new_thing)

        return msg


    def format_thing(self, string, thing, singular='is', plural='are'):
